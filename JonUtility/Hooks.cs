    ...
    public class UserActivityHook
    {
        ...
        public void Start(bool installMouseHook)
        {
            ...
            if (this.hMouseHook == 0 && installMouseHook)
            {
                ...
                UserActivityHook.MouseHookProcedure = this.MouseHookProc;
                //install hook
                this.hMouseHook = UserActivityHook.SetWindowsHookEx(
                    UserActivityHook.WH_MOUSE_LL,
                    UserActivityHook.MouseHookProcedure,
                    IntPtr.Zero,
                    0);
        ...
        private int MouseHookProc(int nCode, int wParam, IntPtr lParam)
        {
            ...
            if ((nCode >= 0) && (this.OnMouseActivity != null))
            {
                ...
                switch (wParam)
                {
                    case UserActivityHook.WM_MOUSEWHEEL:
                ...
                {
                    if (wParam == UserActivityHook.WM_LBUTTONDBLCLK || wParam == UserActivityHook.WM_RBUTTONDBLCLK)
                    {
                        clickCount = 2;
                    }
                    else
